...
def generate_html(subject, comps_scored, output_path, card=None):
    ...
    # Statistics
    ratios = [c["_ratio"] for c in comps_scored if c["_ratio"] is not None]
    prices = [c["_sale_price"] for c in comps_scored]

    median_ratio  = sorted(ratios)[len(ratios) // 2] if ratios else None
    avg_ratio     = sum(ratios) / len(ratios) if ratios else None
    median_price  = sorted(prices)[len(prices) // 2] if prices else None
    above_assessed = [c for c in comps_scored if c["_sale_price"] < subj_assessed]
    high_ratio_comps = [c for c in comps_scored if c["_ratio"] and c["_ratio"] > 1.05]
    ...
    for i, c in enumerate(comps_scored):
        ...
        if ratio and ratio < 0.95:
            flag = '<span title="Sold BELOW assessed value — strong comp for appeal" style="color:#27ae60;margin-left:4px">★</span>'
        if ratio and ratio > 1.10:
            flag = '<span title="Assessed ABOVE sale price — supports your appeal" style="color:#c0392b;margin-left:4px">▲</span>'
        rows += f"""
        <tr>
          <td style="text-align:center;color:#888">{i+1}</td>
          <td><strong>{c.get('Address','')}</strong><br>
              <span style="font-size:0.8em;color:#999">{c.get('CityName','')} · {c.get('NeighborhoodCode','')}</span></td>
          <td>{fmt_date(c.get('DeedDate'))}</td>
          <td style="text-align:right"><strong>{fmt_money(c['_sale_price'])}</strong></td>
          <td style="text-align:right">{fmt_money(int(c.get('TotalMarketValue') or 0))}</td>
          <td style="text-align:center">{fmt_ratio(ratio)}{flag}</td>
...
<p class="note">
  <strong>★ Green</strong> = sold below assessed value — supports lower assessment.
  <strong>▲ Red</strong> = assessed above sale price — the county assessed this property for more than it sold for.<br>
  <strong>Ratio &gt;100%</strong> means assessed value exceeds the actual sale price.
...
def main():
    ...
    # Console summary
    print(f"\n  {'Address':<38} {'Date':<12} {'Sale':>10} {'Assessed':>10} {'Ratio':>8} {'Score':>6}")
    print(f"  {'-'*38} {'-'*12} {'-'*10} {'-'*10} {'-'*8} {'-'*6}")
    for c in comps_scored:
        r = c["_ratio"]
        rs = f"{r*100:.1f}%" if r else "  —"
        print(f"  {c.get('Address',''):<38} {fmt_date(c.get('DeedDate')):<12} "
              f"{fmt_money(c['_sale_price']):>10} "
              f"{fmt_money(int(c.get('TotalMarketValue') or 0)):>10} "
              f"{rs:>8} {c['_score']:>6}")

    prices = [c["_sale_price"] for c in comps_scored]
    ratios = [c["_ratio"] for c in comps_scored if c["_ratio"]]
    if prices:
        median_p = sorted(prices)[len(prices)//2]
        print(f"\n  Median comp sale price: ${median_p:,}")
        if median_p < assessed:
            print(f"  Your assessment is ${assessed - median_p:,} ({(assessed-median_p)/median_p*100:.1f}%) above the median comp sale")
    if ratios:
        median_r = sorted(ratios)[len(ratios)//2]
        print(f"  Median assessment ratio: {median_r*100:.1f}%  (100% = assessed exactly at sale price)")
    ...
    generate_html(subject, comps_scored, out, card=card)
